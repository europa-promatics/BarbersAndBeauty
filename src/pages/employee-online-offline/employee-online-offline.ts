...
export class EmployeeOnlineOffline {
 ...
 ngOnInit(){
  ...
  this.settingsType=localStorage['usertype']
 }
...
enable_notifications() {
        if (this.message_notifications == true) {
            ...
            Observable.fromPromise(loading.present())
                .flatMap(data => this.dataservice.toggleNotifications(localStorage['employeeid'], 1))
                .subscribe(data =>
                    loading.dismiss().then(() => {
                        ...
                        Observable.fromPromise(loading.present())
                            .flatMap(data => this.dataservice.toggleNotifications(localStorage['employeeid'], 0))
                            .subscribe(data =>
                                loading.dismiss().then(() => {
