...
export class CustomerEmployeeListing {
  ...
  public loadingCtrl: LoadingController) {
	...
	this.serviceTitle=this.navParams.get('service')
	//alert('Gagan: '+this.serviceTitle.service_id);
  ...
  ngOnInit(){
    ...
    .subscribe(data =>
      loading.dismiss().then(() =>{ 
		...
		//alert(JSON.stringify(this.lists.employeeinfo));
  ...
  view(employeeid,name,number,category,image,email,status,service){
    if(localStorage['auth']=='false'){
      ...
      employeeimage:image,email:email,ownerbook:this.ownerbook,status:status, service:this.serviceTitle.service_id})
    }
  }
  owner(){
    if(localStorage['auth']=='false'){
      ...
      salonownerimage:this.ownerpic,salonowneremail:this.salonowneremail,ownerbook:this.ownerbook,position:this.position,Salonstatus:this.salonstatus, service:this.serviceTitle.service_id})
    }  
  }
}
