    ...
    Observable.fromPromise(loading.present())
      ...
      .subscribe(data =>{
        ...
        if(data.status==1){
          ...
          this.my_id = this.minedata.bookinginfo[0].transaction_id
          this.cancel_notication_data = this.minedata.cancelinfo
          this.admin_notication_data = this.minedata.admin_message
          this.notifiy_count=this.minedata.bookingcount
          this.status=this.minedata.status
