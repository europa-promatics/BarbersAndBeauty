...
.subscribe(data =>
            ...
            this.mydata=data            
                ...
                this.message_notifications=localStorage['noti_status']
 ...
 this.selected_value6=this.mydata.data.paypal_merchant_signature
 ...
 if(this.available_value=="on dutty"){
  ...
  paypal_settings() {
    let modal = this.modalCtrl.create('SalonPaypalSettingsPage');
    modal.present();
       modal.onDidDismiss(data =>{
         this.mydata=data
        
         if(this.mydata==undefined){
           this.selected_value6='';
         }
         else{
        this.selected_value6=this.mydata
        }
       })
...
enable_notifications() {
        if (this.message_notifications == true) {
            let loading = this.loadingCtrl.create({
                content: 'Loading..'
            });
            Observable.fromPromise(loading.present())
                .flatMap(data => this.dataservice.toggleNotifications(localStorage['salonid'], 1))
                .subscribe(data =>
                    loading.dismiss().then(() => {
                            localStorage['noti_status'] = true;
                            if (data.status == "success") {
                                let alert = this.alertCtrl.create({
                                    title: ' Message Notifications are now enabled.',
                        ...
                        Observable.fromPromise(loading.present())
                            .flatMap(data => this.dataservice.toggleNotifications(localStorage['salonid'], 0))
                            .subscribe(data =>
                                loading.dismiss().then(() => {
                                        ...
                                        if (data.status == "success") {
                                            ...
                                            let alert = this.alertCtrl.create({
                                                title: ' Message Notifications are disabled.',
...
let loading = this.loadingCtrl.create({content: 'Loading...'});
         Observable.fromPromise(loading.present())
         .flatMap(data => this.dataservice.account_settings(this.payment_value2,this.schedule_appointment,this.available_value2,this.my_availability2,this.online_notice,this.online_cancel,this.selected_value6))
         .subscribe(data =>
                    loading.dismiss().then(() =>{ 
                      if(data.status==1){
   let alert = this.alertCtrl.create({
