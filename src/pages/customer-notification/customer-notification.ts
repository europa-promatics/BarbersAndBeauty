...
decline_data
  ...
  ionViewWillEnter(){
    this.dataservice.setNotificationSeen(localStorage['customerid']).subscribe(data=>{
    },err=>{
    })
         ...
         .subscribe(data =>
                    loading.dismiss().then(() =>{ 
                        ...
                        console.log("accept"+ this.notificationdata)
                         this.declined=this.notification.admin_message.reverse();
...
changecolor(paymentid){
    let loading = this.loadingCtrl.create({content: 'Please Wait...'});
         Observable.fromPromise(loading.present())
         .flatMap(data => this.dataservice.Changenotificationcolor(paymentid))
         .subscribe(data =>
...
pay(payment_id,cost,online_booking_staus){
