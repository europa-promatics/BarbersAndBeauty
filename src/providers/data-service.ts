...
add_appointment(service_id,starttime,employee_id,salon_id,customer_id,selecteddate): Observable<any>{

 return this.http.post(ENV.mainApi+'/customBooking.json',{
...
getSpecificService(saloon_id):Observable<any>{
if(localStorage['usertype']==2){
return this.http.post(ENV.mainApi+'/fetchServices.json',{
salon_id:localStorage['salonid'],
...
MarkRead(id, status):Observable<any>{
return this.http.post(ENV.mainApi+'/adminNotificationSeen.json',{
		id:id,
		message_seen:status
})
...
CheckService(service_id, employee_id, type){

return this.http.post(ENV.mainApi+'/employeeBookAvailability.json',{
service_id:service_id,
employee_id:employee_id,
type:type
...
myclient(){
	...
	var link;
	 if(localStorage['usertype']==2) {
		 ...
		 link="/customerAppointmentByEmployeeId.json"
		   return this.http.post(ENV.mainApi+link,{
			employee_id: localStorage['employeeid']
			}).timeout(ENV.timeout)
    .map((data) =>{
     return data.json()
    ...
    .catch(error =>{
     return error
		 ...
		 link="/customerAppointmentBySalonIdLastDate.json"
		 return this.http.post(ENV.mainApi+link,{
      salon_id:localStorage['salonid']
...
hours_notice_for_online_booking,
 hours_notice_for_cancel_booking,paypal_credentials){
return this.http.post(ENV.mainApi+'/accountSettings.json',{
...
EmployeeCancelBooking(paymentid,paystatus,status_change){
...
EmployeeConfirmBooking(paymentid,paystatus,status_change){
   return this.http.post(ENV.mainApi+'/employeeBookingSuccessAndCancelStatusUpdated.json',{
                               payment_id:paymentid,
...
Changenotificationcolor(paymentid){
...
employe_side_notification_color_change(mypayment_id){
...
toggleNotifications(user_id,noti_status){
	return this.http.post(ENV.mainApi+'/toggleNotifications.json',{
		user_id:user_id,
